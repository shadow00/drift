...
manual_throttle = [f"t{num}" for num in range(1100, 2001, 50)]  # Small steps (18)
# manual_throttle = [f"t{num}" for num in range(1100, 1901, 50)]  # Small steps
# manual_throttle = [f"t{num}" for num in range(1100, 1301, 50)]  # Small steps
# strings_to_send = ["a", *manual_throttle, *manual_throttle[::-1], "s"]
# strings_to_send = ["a", *manual_throttle, "D2", *manual_throttle[::-1], "s"]  # Ramp up and down, with extra 2s delay in the middle
strings_to_send = ["a", *manual_throttle, *manual_throttle[::-1], "s"]  # Ramp up and down
# strings_to_send = ["a", *manual_throttle, "t1100", "s"]  # Ramp up, then idle and stop
# delay = 0.2
...
expected_time = sum([int(s[1:]) if s[0]=="D" else arm_delay if s[0]=="a" else delay for s in strings_to_send])
...
for string in strings_to_send:
    ...
    if string[0] == "a":  # If we just sent the arm command, sleep for 'arm_delay' seconds instead of the regular delay
        now = time.time()
